...
Scriptoria is primarily designed to create a text-based workflow for video editors that involves using AI to structure video captions into an interactive HTML text transcript. The transcript can then be marked up and arranged into a script to aide with video outlining and narrative structuring. There is functionality that allows for your script to easily link back with the captions in Adobe Premiere Pro, and a customized Gemini API assistant. 
