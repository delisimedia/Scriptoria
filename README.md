...
## Theme-based highlighting system, annotating, script organization. Gemini API Integration
...
In addition to its video-production tailored feature-set, Scriptoria has also been designed so that it can be utilized by the reader as an academic aide to reading. The them-based highlighting, tagging, and note-taking systems make for a robust interactive e-reader. Additionally, the Gemini API integration allows you to ask questions to Gemini about any selected text instantly without leaving the program.
